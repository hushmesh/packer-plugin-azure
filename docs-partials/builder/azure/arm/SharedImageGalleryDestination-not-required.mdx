...
- `uefi_signature_template_names` ([]string) - A list of UEFI signature template names.

- `uefi_additional_signatures_pk` ([]string) - Additional platform key UEFI signatures. PEM encoded x509 certificates.

- `uefi_additional_signatures_kek` ([]string) - Additional key exchange key UEFI signatures. PEM encoded x509 certificates.

- `uefi_additional_signatures_db` ([]string) - Additional database UEFI signatures. PEM encoded x509 certificates.

- `uefi_additional_signatures_dbx_x509` ([]string) - Additional database revocation UEFI signatures. PEM encoded x509 certificates.

- `uefi_additional_signatures_dbx_sha256` ([]string) - Additional database revocation UEFI signatures. Base64 encoded SHA256 hashes.
